...
class ProxyUpdate {
    ...
    public update(key:string, data:any):void {
        ...
        if (data.hasOwnProperty("c")) {
            var cdata:any = data["c"];//把c字段的缓存数据取出
            var keys = Object.keys(cdata);//查找所有这个字段
            for (var i:number = 0, len = keys.length; i < len; i++) {
                var k1 = keys[i];
                if (this._cache[k1]) {//缓存中存在
                    this._update(this._cache[k1], cdata[k1]);//更新
                    App.MessageCenter.dispatch(k1 + "_HttpUpdate");//回调
                }
            }
        }
    }
